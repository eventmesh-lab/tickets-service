...
7. **Capacidad del evento:** No se pueden generar/confirmar más tickets que la capacidad disponible del evento. La capacidad proviene del `events-service` y puede ser total o por sección (según `Seccion`).
8. **Estado del evento:** La emisión/venta procede solo si el evento está `Publicado` y la validación si está `EnCurso` o según la ventana definida.
...
### Events Service (capacidad y estado)

- Lecturas sincrónicas para: estado del evento (`Publicado`, `EnCurso`), capacidad total y por sección, y (si aplica) mapa de secciones/asientos.
- Cálculo de disponibilidad: `capacidad - (confirmados + pendientes reservados)`.
- Prevención de sobreventa: transacciones + restricción de unicidad por `asiento_id` cuando hay asientos numerados.

### Users Service (identidad del asistente)

- Validar existencia y estado del `AsistenteId` (usuario activo) al generar/confirmar.
- Enriquecer datos del QR/notificación (opcional) con email/nombre.
...
-- Evitar duplicado de asiento activo (aprox.)
...
### GET /api/tickets/evento/{eventoId}/disponibilidad

Disponibilidad actual considerando capacidad de `events-service`.
...
- [Reservations Service](reservations-service.md)
